...
export const RegistrationsClient: FC<RegistrationsClientProps> = ({
  ...
  const visibleEvents = useMemo(() => {
    const filtered = activeOrgId
      ? events.filter((e) =>
          e.organizations.some((o) => o.orgId === activeOrgId),
        )
      : events;
    return filtered;
  }, [events, activeOrgId]);
  ...
  useMemo(() => {
    if (
      typeFilter !== 'ALL' &&
      !selectedEvent?.registrantTypes.some((t) => t.id === typeFilter)
    ) {
      setTypeFilter('ALL');
    }
  ...
  const registrations = eventSelected ? (fetchedRegistrations ?? []) : [];
  ...
  const selectedEventTitle = visibleEvents.find(
    (e) => e.id === eventFilter,
  )?.title;
                    ...
                    return (
                      ...
                      >
                        ...
                        <TableCell>
                          <Badge
                            variant='outline'
                            className={cn('gap-1 text-xs', s.className)}
                          >
        ...
        (() => {
          return (
            <WalkInRegistrationDialog
              ...
              eventOrgs={
                selectedEvent?.organizations.filter(
                  (o) => o.inviteStatus === 'ACCEPTED',
                ) ?? []
              }
            />
          );
      ...
      {importOpen &&
        ...
        (() => {
          const existingEmails = new Set(
            registrations
              .map((r) => r.registrant.email?.toLowerCase())
