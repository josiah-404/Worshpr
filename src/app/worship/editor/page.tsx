...
import {
  ...
  Tv2, MonitorOff, ArrowLeft, Save, Check, LayoutGrid, SlidersHorizontal,
} from "lucide-react";
import { BACKGROUNDS, TRANSITIONS, FONTS, SIZES, SPEEDS, PREVIEW_FONT_SIZES, CONTROLLER_FONT_SIZES } from "@/lib/worship-constants";
...
function BgThumb({ bgCls, label, selected, onClick }: { bgCls: string; label: string; selected: boolean; onClick: () => void }) {
  const ref = useRef<HTMLButtonElement>(null);
  const [scale, setScale] = useState(0.1);
  useEffect(() => {
    const el = ref.current;
    if (!el) return;
    const obs = new ResizeObserver(([e]) => setScale(e.contentRect.width / 1920));
    obs.observe(el);
  ...
  return (
    ...
    >
      <div className={bgCls} style={{ position: "absolute", width: "1920px", height: "1080px", top: "50%", left: "50%", transform: `translate(-50%, -50%) scale(${scale})`, transformOrigin: "center" }} />
      <span className="absolute inset-0 flex items-center justify-center text-[9px] font-semibold text-white drop-shadow-md" style={{ zIndex: 1 }}>{label}</span>
    </button>
  );
}
      ...
      <div className="flex flex-col gap-4 flex-1 min-h-0">
        ...
        <div className="flex gap-5 min-h-0 flex-1">
          ...
          <div className="w-56 shrink-0 flex flex-col gap-3 min-h-0">
            <p className="text-[11px] font-medium text-muted-foreground/60 uppercase tracking-widest shrink-0">Slides</p>
            ...
            <div className="shrink-0 pb-1">
              <p className="text-[11px] font-medium text-muted-foreground/50 uppercase tracking-widest mb-1.5">Up Next</p>
              <SlidePreview
                slide={nextSlide}
                bgCls={bgCls}
        ...
        <SettingsDrawer
          ...
          transSpeed={transSpeed} animSpeed={animSpeed}
          mode={mode}
          onChangeBg={changeBg} onChangeTr={changeTr} onChangeFont={changeFont} onChangeSize={changeSize}
          onChangeTransSpeed={changeTransSpeed} onChangeAnimSpeed={changeAnimSpeed}
          onOpenBgPicker={() => setBgDialogOpen(true)}
          onOpenPresenter={openPresenter} onEndPresentation={endPresentation}
        />
      </div>
    );
  }
      ...
      <div className="flex gap-5 flex-1 min-h-0">
        ...
        <div className="w-52 shrink-0 flex flex-col gap-3 min-h-0">
          <p className="text-[11px] font-medium text-muted-foreground/60 uppercase tracking-widest shrink-0">
            Slides {slides.length > 0 && <span className="text-muted-foreground/40">({slides.length})</span>}
          </p>
        ...
        <div className="w-96 shrink-0 flex flex-col gap-3">
          <div className="flex items-center justify-between shrink-0">
            <div>
              ...
              <p className="text-xs text-muted-foreground mt-0.5">Separate slides with a blank line</p>
            </div>
          </div>
          <textarea
            value={lyrics}
        ...
        <div className="flex-1 min-w-0 flex flex-col gap-4 overflow-y-auto min-h-0">
          ...
          <div className="space-y-1.5 shrink-0">
            <p className="text-[11px] font-medium text-muted-foreground/60 uppercase tracking-widest">Preview</p>
            <SlidePreview
              slide={currentSlide}
            ...
            {slides.length > 0 && (
              <div className="flex items-center justify-between gap-2 pt-0.5">
                ...
                <span className="text-[11px] text-muted-foreground">{current + 1} / {slides.length}</span>
                <button
                  onClick={() => goTo(current + 1)}
                  disabled={current === slides.length - 1}
                  className="flex items-center gap-1 rounded-md border border-border px-2.5 py-1 text-xs text-muted-foreground hover:bg-accent/60 hover:text-foreground transition-colors disabled:opacity-30 disabled:cursor-not-allowed"
          ...
          <div className="space-y-2 shrink-0">
            <div className="flex items-center justify-between">
              <p className="text-[11px] font-medium text-muted-foreground/60 uppercase tracking-widest">Background</p>
              <button
          ...
          {/* Transition */}
          <div className="space-y-2 shrink-0">
            <p className="text-[11px] font-medium text-muted-foreground/60 uppercase tracking-widest">Transition</p>
          ...
          <div className="grid grid-cols-2 gap-4 shrink-0">
            <div className="space-y-2">
              <p className="text-[11px] font-medium text-muted-foreground/60 uppercase tracking-widest">Transition Speed</p>
            ...
            <div className="space-y-2">
              <p className="text-[11px] font-medium text-muted-foreground/60 uppercase tracking-widest">Animation Speed</p>
              <div className="flex flex-col gap-1.5">
                {SPEEDS.map((s) => (
                  <button
                    key={s.id}
          ...
          <div className="grid grid-cols-2 gap-4 shrink-0 pb-4">
            <div className="space-y-2">
              <p className="text-[11px] font-medium text-muted-foreground/60 uppercase tracking-widest">Font</p>
            ...
            <div className="space-y-2">
              <p className="text-[11px] font-medium text-muted-foreground/60 uppercase tracking-widest">Text Size</p>
      ...
      <div className="flex gap-5 flex-1 min-h-0">
        <div className="w-52 shrink-0 flex flex-col gap-3">
          ...
          {Array.from({ length: 5 }).map((_, i) => (
            <Skeleton key={i} className="w-full rounded-xl" style={{ aspectRatio: "16/9" }} />
          ))}
        </div>
        <div className="w-96 shrink-0 flex flex-col gap-3">
          <div className="flex items-center justify-between">
